...
/**
 * UBL 2.1 Invoice Generator Service
 *
 * Generates UBL 2.1 compliant Invoice XML documents
 * following Peppol BIS Billing 3.0 specification.
 */
@Injectable()
export class UblInvoiceGeneratorService {
   ...
   */
  generateInvoice(invoice: UblInvoice): string {
    const lines: string[] = [];

    lines.push('<?xml version="1.0" encoding="UTF-8"?>');
    lines.push(
      this.openTag('Invoice', {
        xmlns: this.UBL_NS,
    ...
    // Header elements
    lines.push(
      this.cbcElement('CustomizationID', invoice.customizationId || this.CUSTOMIZATION_ID),
    );
    lines.push(this.cbcElement('ProfileID', invoice.profileId || this.PROFILE_ID));
    lines.push(this.cbcElement('ID', invoice.id));
    lines.push(this.cbcElement('IssueDate', invoice.issueDate));

    if (invoice.dueDate) {
      lines.push(this.cbcElement('DueDate', invoice.dueDate));
    }

    lines.push(this.cbcElement('InvoiceTypeCode', invoice.invoiceTypeCode));
    ...
    if (invoice.taxPointDate) {
      lines.push(this.cbcElement('TaxPointDate', invoice.taxPointDate));
    }

    lines.push(this.cbcElement('DocumentCurrencyCode', invoice.documentCurrencyCode));

    if (invoice.taxCurrencyCode) {
      lines.push(this.cbcElement('TaxCurrencyCode', invoice.taxCurrencyCode));
    }

    if (invoice.accountingCost) {
      lines.push(this.cbcElement('AccountingCost', this.escapeXml(invoice.accountingCost)));
    }

    if (invoice.buyerReference) {
      lines.push(this.cbcElement('BuyerReference', this.escapeXml(invoice.buyerReference)));
    }

    // Invoice Period
    if (invoice.invoicePeriod) {
      lines.push(this.generatePeriod(invoice.invoicePeriod));
    }

    // Order Reference
    if (invoice.orderReference) {
      lines.push(this.openTag('cac:OrderReference'));
      lines.push(this.cbcElement('ID', invoice.orderReference.id));
      if (invoice.orderReference.salesOrderId) {
        lines.push(this.cbcElement('SalesOrderID', invoice.orderReference.salesOrderId));
      }
      lines.push(this.closeTag('cac:OrderReference'));
    }

    // Billing Reference
    if (invoice.billingReference?.length) {
      for (const ref of invoice.billingReference) {
        lines.push(this.generateBillingReference(ref));
      }
    }

    // Document References
    if (invoice.despatchDocumentReference) {
      lines.push(
    ...
    if (invoice.receiptDocumentReference) {
      lines.push(
        this.generateDocumentReference(
    ...
    if (invoice.originatorDocumentReference) {
      lines.push(
        this.generateDocumentReference(
    ...
    if (invoice.contractDocumentReference) {
      lines.push(
        this.generateDocumentReference(
    ...
    if (invoice.additionalDocumentReference?.length) {
      for (const ref of invoice.additionalDocumentReference) {
    ...
    if (invoice.projectReference) {
      ...
      lines.push(this.cbcElement('ID', invoice.projectReference.id));
    ...
    lines.push(this.generateParty(invoice.accountingSupplierParty.party));
    ...
    if (invoice.paymentTerms) {
      ...
      if (invoice.paymentTerms.note?.length) {
        for (const note of invoice.paymentTerms.note) {
          lines.push(this.cbcElement('Note', this.escapeXml(note)));
        }
      }
    ...
    if (invoice.allowanceCharge?.length) {
      for (const ac of invoice.allowanceCharge) {
    ...
    lines.push(this.generateLegalMonetaryTotal(invoice.legalMonetaryTotal));

    // Invoice Lines
    for (const line of invoice.invoiceLine) {
      lines.push(this.generateInvoiceLine(line));
    }

    lines.push(this.closeTag('Invoice'));
   ...
   */
  private generateInvoiceLine(line: UblInvoiceLine): string {
    const lines: string[] = [];
    lines.push(this.openTag('cac:InvoiceLine'));
    ...
    lines.push(this.closeTag('cac:InvoiceLine'));
    return lines.join('\n');
  }
